    ...
    <div className="flex-1 p-6 md:p-8 w-full pb-32 space-y-8">
      ...
      <div className="relative overflow-hidden rounded-xl border border-primary/30 bg-surface-container-high p-6 shadow-[0_0_30px_rgba(242,202,80,0.1)]">
        ...
        <div className="relative z-10">
          ...
          <h1 className="font-display-lg text-[36px] md:text-[48px] text-primary-fixed gold-glow leading-tight">{explorerName}</h1>
        </div>
      </div>

      {/* Empty Chronicle State */}
      {isEmpty ? (
        <div className="text-center py-16 bg-surface-container-high border border-outline-variant/40 rounded-xl">
          <p className="text-[48px] mb-4">📜</p>
          <h2 className="font-display-lg text-[24px] text-primary-fixed gold-glow mb-2">Your Chronicle is Empty</h2>
          <p className="text-on-surface-variant mb-6">Begin your first expedition to start recording your adventures.</p>
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/dashboard/groups" className="bg-primary/10 border border-primary/50 text-primary px-6 py-2.5 rounded-lg font-label-sm text-xs uppercase tracking-widest hover:bg-primary/20 transition-all flex items-center gap-2">
              <span className="material-symbols-outlined text-[16px]">explore</span> Forge First Expedition
            </Link>
            <Link href="/dashboard/expenses" className="bg-secondary/10 border border-secondary/50 text-secondary px-6 py-2.5 rounded-lg font-label-sm text-xs uppercase tracking-widest hover:bg-secondary/20 transition-all flex items-center gap-2">
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={stat.label} className={`bg-surface-container-high border border-outline-variant rounded-xl p-4 ${stat.glow}`}>
                <div className="flex items-start justify-between mb-3">
                  <span className={`material-symbols-outlined text-[24px] ${stat.color}`}>{stat.icon}</span>
                  <div className={`w-1.5 h-1.5 rounded-full ${stat.color} opacity-60 candle-flicker`} />
                </div>
                <p className={`font-display-lg text-[24px] md:text-[28px] ${stat.color} leading-none`}>{stat.value}</p>
                <p className="font-label-sm text-[10px] text-on-surface-variant uppercase tracking-widest mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-headline-lg text-[16px] text-on-surface-variant uppercase tracking-widest flex items-center gap-2">
                <span className="material-symbols-outlined text-[16px]">bolt</span> Quick Actions
              </h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {QUICK_ACTIONS.map(action => (
                <Link key={action.label} href={action.href} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-xl border bg-surface-container-low ${action.color} transition-all duration-200 hover:scale-[1.02] group`}>
                  <span className="material-symbols-outlined text-[26px] group-hover:scale-110 transition-transform">{action.icon}</span>
                  <span className="font-label-sm text-[10px] uppercase tracking-widest text-center">{action.label}</span>
                </Link>
              ))}
            </div>
          </div>
